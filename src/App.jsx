...
const App = () => {
  ...
  const handleEnd = () => {
    if (isDragging.current) {
      ...
      if (delta > 50) {
        if (index === 0 && reelRefs.current[index].scrollHeight > reelRefs.current[index].clientHeight) {
          const currentScrollPosition = reelRefs.current[index].scrollTop;
          const scrollBottom = reelRefs.current[index].scrollHeight - reelRefs.current[index].clientHeight;
